...
class HTMLResponse(requests.Response):
    ...
    def _from_response(cls, response, session: Union['HTMLSession', 'AsyncHTMLSession']):
        """Create an HTMLResponse from a plain requests.Response.

        Copies all attributes from the original response into a new
        HTMLResponse instance so that ``.html`` is available.
        """
...
class BaseSession(requests.Session):
    ...
    def request(self, *args, **kwargs):
        """Make an HTTP request and ensure an HTMLResponse is returned.

        Overrides :meth:`requests.Session.request` so that even when the
        response-hook return value is silently discarded by certain
        versions of the ``requests`` library, callers always receive an
        :class:`HTMLResponse` with the ``.html`` attribute available.
        """
        response = super().request(*args, **kwargs)
        if isinstance(response, HTMLResponse):
            return response
        # Hook was not applied -- wrap manually.
        if not response.encoding:
            response.encoding = DEFAULT_ENCODING
        return HTMLResponse._from_response(response, self)
